...
rx.on("friendRequest", function(e) {
  console.log("[rx] Accepting friend request from: %s",
              e.publicKeyHex().toUpperCase());
  rx.addFriendNoRequestSync(e.publicKey());
});

rx.on("friendConnectionStatus", function(e) {
  console.log("[rx] Friend connection status: %s",
...
var packetCallback = function(e) {
  ...
  if (e.isLossless()) {
    rx.sendLosslessPacketSync(e.friend(), e.id(),
                              Buffer.from("lossless-receipt-packet-content"));
  } else {
    rx.sendLossyPacketSync(e.friend(), e.id(),
                           Buffer.from("lossy-receipt-packet-content"));
  }
};
...
tx.on("friendConnectionStatus", function(e) {
  console.log("[tx] Friend connection status: %s",
              e.isConnected() ? "online" : "offline");
  if (e.isConnected()) {
    console.log("[tx] Sending lossless + lossy packets");
    tx.sendLosslessPacketSync(e.friend(), LOSSLESS_CHANNEL,
                              Buffer.from("hello-world-lossless"));
    tx.sendLossyPacketSync(e.friend(), LOSSY_CHANNEL,
                           Buffer.from("hello-world-lossy"));
...
[{tox : tx, name : "tx"},
 {tox : rx, name : "rx"},
  ...
  nodes.forEach(function(node) {
    ...
    console.log("[%s] Successfully bootstrapped from %s at %s:%d", toxName,
                node.maintainer, node.address, node.port);
    console.log("... with key %s", node.key);
  });

  tox.on("selfConnectionStatus", function(e) {
    console.log("[%s] %s", toxName,
                e.isConnected() ? "Connected" : "Disconnected");
  });

  console.log("[%s] Address: %s", toxName,
              tox.getAddressHexSync().toUpperCase());
