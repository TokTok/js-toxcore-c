...
var initProfile = function(callback) {
  var setName = tox.setNameAsync("Promises Bot"),
      setStatusMessage =
          tox.setStatusMessageAsync("node-toxcore promises bot example");
...
var initCallbacks = function(callback) {
  ...
  tox.on("friendConnectionStatus", function(e) {
    console.log("Friend[" + e.friend() + "] is now " +
                (e.isConnected() ? "online" : "offline"));
  });

  tox.on("friendTyping", function(e) {
    console.log("Friend[" + e.friend() + "] is " +
    ...
    if (e.message() === "namelen") {
      var getNameSize = tox.getFriendNameSizeAsync(e.friend()),
          getStatusMessageSize =
              tox.getFriendStatusMessageSizeAsync(e.friend());
      Promise.join(getNameSize, getStatusMessageSize,
                   function(nameSize, statusMessageSize) {
