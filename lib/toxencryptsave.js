...
ToxEncryptSave.prototype._createLibrary = function(libpath) {
  ...
  return ffi.Library(libpath, {
    ...
    tox_get_salt :
        [ "bool", [ UInt8Ptr, UInt8Ptr, ref.refType(TOX_ERR_GET_SALT) ] ],
    tox_is_data_encrypted : [ "bool", [ UInt8Ptr ] ],
    tox_pass_decrypt : [
      "bool",
      [
        UInt8Ptr, "size_t", UInt8Ptr, "size_t", UInt8Ptr,
        ref.refType(TOX_ERR_DECRYPTION)
      ],
    ],
    tox_pass_encrypt : [
      "bool",
      [
        UInt8Ptr, "size_t", UInt8Ptr, "size_t", UInt8Ptr,
        ref.refType(TOX_ERR_ENCRYPTION)
      ],
    ],
    tox_pass_key_decrypt : [
      "bool",
      [
        ref.refType(ToxPassKey), UInt8Ptr, "size_t", UInt8Ptr,
        ref.refType(TOX_ERR_DECRYPTION)
      ],
    ],
...
ToxEncryptSave.prototype.decrypt = function(data, pass, callback) {
  ...
  this.getLibrary().tox_pass_decrypt.async(
      data, data.length, pass, pass.length, out, eptr, function(err, success) {
...
ToxEncryptSave.prototype.decryptSync = function(data, pass) {
  ...
  var eptr = ref.alloc(TOX_ERR_DECRYPTION),
      out = Buffer.alloc(data.length - consts.TOX_PASS_ENCRYPTION_EXTRA_LENGTH),
      success = this.getLibrary().tox_pass_decrypt(data, data.length, pass,
                                                   pass.length, out, eptr);
...
ToxEncryptSave.prototype.encryptSync = function(data, pass) {
  ...
  var eptr = ref.alloc(TOX_ERR_ENCRYPTION),
      out = Buffer.alloc(data.length + consts.TOX_PASS_ENCRYPTION_EXTRA_LENGTH),
      success = this.getLibrary().tox_pass_encrypt(data, data.length, pass,
                                                   pass.length, out, eptr);
...
ToxEncryptSave.prototype.getSalt = function(data, callback) {
  ...
  this.getLibrary().tox_get_salt.async(data, salt, eptr,
 ...
 */
ToxEncryptSave.prototype.isDataEncryptedSync = function(
    data) { return this.getLibrary().tox_is_data_encrypted(data); };
...
ToxEncryptSave.prototype.encryptPassKey = function(data, passKey, callback) {
  ...
  this.getLibrary().tox_pass_key_encrypt.async(
      passKey.ref(), data, data.length, out, eptr, function(err, success) {
        var terr = errors.encryption(eptr.deref());
        if (!err && terr)
          err = terr;
        if (!err && !success)
...
ToxEncryptSave.prototype.encryptPassKeySync = function(data, passKey) {
  var out = Buffer.alloc(data.length + consts.TOX_PASS_ENCRYPTION_EXTRA_LENGTH),
      eptr = ref.alloc(TOX_ERR_ENCRYPTION),
      success = this.getLibrary().tox_pass_key_encrypt(passKey.ref(), data,
                                                       data.length, out, eptr);
  var err = errors.encryption(eptr.deref());
  if (err)
    throw err;
  else if (!success)
...
ToxEncryptSave.prototype.decryptPassKeySync = function(data, passKey) {
  var out = Buffer.alloc(data.length - consts.TOX_PASS_ENCRYPTION_EXTRA_LENGTH),
      eptr = ref.alloc(TOX_ERR_DECRYPTION),
      success = this.getLibrary().tox_pass_key_decrypt(passKey.ref(), data,
                                                       data.length, out, eptr);
  var err = errors.decryption(eptr.deref());
  if (err)
    throw err;
 ...
 */
ToxEncryptSave.prototype.encryptFile = function(filepath, data, pass,
                                                callback) {
 ...
 * @return {ffi.Library}
 */
ToxEncryptSave.prototype.getLibrary = function() { return this._library; };
...
ToxEncryptSave.prototype._performDerive = function(opts) {
  var api = opts["api"], pass = opts["pass"], salt = opts["salt"],
      useSalt = opts["useSalt"], async = opts["async"],
      callback = opts["callback"];

  if (_.isString(pass))
    pass = Buffer.from(pass);
  var out = ref.alloc(ToxPassKey), eptr = ref.alloc(TOX_ERR_KEY_DERIVATION);
  ...
  if (async) {
    ...
    var success = api(), err = errors.keyDerivation(eptr.deref());
    if (err)
      throw err;
    else if (!success)
      throw errors.unsuccessful();
 ...
 * @param {Error} error - error, if any
 * @param {ToxPassKey} passKey - ToxPassKey struct to use in other
 *     ToxEncryptSave methods
 */
