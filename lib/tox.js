...
Tox.prototype.createLibrary = function (libpath) {
  ...
  return ffi.Library(libpath, {
    ...
    tox_file_control: [
      "bool",
      [ToxPtr, "uint32", "uint32", TOX_FILE_CONTROL, ref.refType(TOX_ERR_FILE_CONTROL)],
    ],
    ...
    tox_friend_send_lossless_packet: [
      "bool",
      [ToxPtr, "uint32", UInt8Ptr, "size_t", ref.refType(TOX_ERR_FRIEND_CUSTOM_PACKET)],
    ],
...
Tox.prototype.getFriendList = function (callback) {
  ...
  this.getFriendListSize(function (err, size) {
    if (!err) {
      ...
      _this
        .getLibrary()
        .tox_self_get_friend_list.async(_this.getHandle(), arr.buffer, function (err) {
          // RefArray -> Javascript Array
          // @todo Make this RefArrayToArray()?
          var nums = [];
          if (!err) {
            for (var i = 0; i < arr.length; i++) nums.push(arr[i]);
          }
...
Tox.prototype.setTypingSync = function (friend, typing) {
  this._checkHandleSync();
  var eptr = ref.alloc(TOX_ERR_SET_TYPING),
    success = this.getLibrary().tox_self_set_typing(this.getHandle(), friend, typing, eptr);
...
Tox.prototype.hash = function (data, callback) {
  ...
  this.getLibrary().tox_hash.async(hash, data, data.length, function (err, success) {
    if (!err && !success) err = errors.unsuccessful();
    if (callback) {
      callback(err, hash);
    }
...
Tox.prototype.seekFileSync = function (friendnum, filenum, position) {
  this._checkHandleSync();
  var eptr = ref.alloc(TOX_ERR_FILE_SEEK),
    success = this.getLibrary().tox_file_seek(this.getHandle(), friendnum, filenum, position, eptr);
 ...
 */
Tox.prototype.hasCrypto = function () {
  return this._crypto instanceof ToxEncryptSave;
};
...
Tox.prototype._createToxOptions = function (opts) {
  ...
  if (data) {
    if (this.hasCrypto() && _.isFunction(pass) && this.crypto().isDataEncryptedSync(data)) {
      var passphrase = pass();
...
Tox.prototype._fixSendMessageArgs = function (to, message, type) {
  ...
  if (_.isBoolean(type)) {
    type = type ? consts.TOX_MESSAGE_TYPE_ACTION : consts.TOX_MESSAGE_TYPE_NORMAL;
  } else if (_.isString(type)) {
    type = /^action$/i.test(type) ? consts.TOX_MESSAGE_TYPE_ACTION : consts.TOX_MESSAGE_TYPE_NORMAL;
...
Tox.prototype._storeFFICallback = function (key, callback) {
  if (!this._ffiCallbacks) this._ffiCallbacks = {};

  if (this._ffiCallbacks[key] === undefined) this._ffiCallbacks[key] = [callback];
...
Tox.prototype._initFriendStatusMessageCb = function () {
  ...
  this._initCallback({
    ...
    wrapper: function (handle, friend, buffer, size, userdata) {
      ...
      _this._emit(
        "friendStatusMessage",
        new toxEvents.FriendStatusMessageEvent(friend, statusMessage)
      );
    },
  });
};
...
Tox.prototype._initFileChunkRequestCb = function () {
  ...
  this._initCallback({
    ...
    wrapper: function (handle, friend, file, position, length, userdata) {
      _this._emit(
        "fileChunkRequest",
        new toxEvents.FileChunkRequestEvent(friend, file, position, length)
      );
    },
  });
};
