...
function loadModule(mod) {
  try {
    return require(
        path.join("js-toxcore-c", "js-toxcore-c", "js-toxcore-c", "lib", mod));
  } catch (e) {
...
describe("Tox", function() {
  ...
  var toxCustomPort =
      new Tox({startPort : customPort, endPort : customPort + 100});
  toxCustomPort.start();
  ...
  describe("friend getter functions", function() {
    describe("before setting", function() {
      describe(
          "#getFriendByPublicKey(), #getFriendByPublicKeySync()", function() {
            it("should throw an exception", function() {
              (function() { tox.getFriendByPublicKeySync(fakePublicKeys[0]); }
                   .should.throw());
            });
      ...
      describe("#deleteFriend(), #deleteFriendSync()", function() {
        ...
        it('should return an exception if friend doesn"t exist',
           function(done) {
      ...
      describe("#hasFriend(), #hasFriendSync()", function() {
        it('should return false if friend doesn"t exist',
           function() { tox.hasFriendSync(0).should.be.false; });

        it('should return false if friend doesn"t exist (async)',
           function(done) {
    ...
    describe("#getFriendPublicKey(), #getFriendPublicKeySync()", function() {
      it("should return a number if a friend has the public key", function() {
        ...
        (function() { tox.getFriendByPublicKeySync(publicKey); }
             .should.throw());
        // Add and try again
        ...
        retrievedKey.toString("hex").toLowerCase().should.equal(
      ...
      it("should return a number if a friend has the public key (async)",
         function(done) {
    ...
    describe(
        "#getFriendPublicKeyHex(), #getFriendPublicKeyHexSync()", function() {
    ...
    describe("#deleteFriend(), #deleteFriendSync()", function() {
      it("should remove an existing friend",
         function() { tox.deleteFriendSync(0); });

      it("should remove an existing friend (async)",
         function(done) { tox.deleteFriend(1, done); });
    });
  });

  describe("#setName(), #setNameSync(), #getName(), #getNameSync()",
           function() {
  ...
  describe(
      "#setStatusMessage(), #setStatusMessageSync(), #getStatusMessage(), #getStatusMessageSync()",
      function() {
  ...
  describe("#setStatus(), #setStatusSync(), #getStatus(), #getStatusSync()",
           function() {
  ...
  describe(
      "#addFriend(), #addFriendSync(), #addFriendNoRequest(), #addFriendNoRequestSync()",
      function() {
        it("should add a friend from a hex string address", function() {
          var addr = fakeAddresses[0], friend = tox.addFriendSync(addr, "Hi!");
          should(friend).be.type("number");
        });

        it("should add a friend from a hex string address (async)",
           function(done) {
        ...
        it("should add a friend from a hex string public key (async)",
           function(done) {
  ...
  describe("#getUdpPort(), #getUdpPortSync()", function() {
    ...
    it("should return a Number with udp enabled by default (async)",
       function(done) {
    ...
    it("should return a not-bound error if not listening on udp (async)",
       function(done) {
  ...
  describe("#getTcpPort(), #getTcpPortSync()", function() {
    it("should throw a not-bound error if not a tcp relay",
       function() { (function() { tox.getTcpPortSync(); }.should.throw()); });

    it("should return a not-bound error if not a tcp relay (async)",
       function(done) {
  ...
  describe("#isUdp()", function() {
    it("should return true with udp enabled by default",
       function() { tox.isUdp().should.be.true; });

    it("should return false if not listening on an udp port",
       function() { toxNoUdp.isUdp().should.be.false; });
  });

  describe("#isTcp()", function() {
    it("should return false if not a tcp relay",
       function() { tox.isTcp().should.be.false; });
  });

  describe(
      "#getSavedataSize(), #getSavedataSizeSync(), #getSavedata(), #getSavedataSync()",
      function() {
        ...
        it("should save data to a Buffer of the specified size (async)",
           function(done) {
        ...
        it("should load the savedata correctly", function() {
          var toxToSave = new Tox(), address = toxToSave.getAddressHexSync(),
              data = toxToSave.getSavedataSync();
          new Tox({data : data})
              .getAddressHexSync()
              .toUpperCase()
              .should.equal(address.toUpperCase());
        });
      });
  ...
  describe("crypto support", function() {
    it("should be provided by default",
       function() { tox.hasCrypto().should.be.true; });

    it('should not break when the "crypto" option is passed to Tox constructor',
       function() {
    ...
    it("should automatically try to decrypt provided encrypted data, given a pass",
       function() {
         this.timeout(5000);
         var toxToSave = new Tox(), address = toxToSave.getAddressHexSync(),
             data = toxToSave.getSavedataSync(), passphrase = "helloWorld",
             edata = toxToSave.crypto().encryptSync(data, passphrase),
             toxEncrypted = [
               new Tox({data : edata, pass : passphrase}),
               new Tox({
                 data : edata,
                 pass : function() { return passphrase; },
               }),
             ];
         toxEncrypted.forEach(function(tox) {
           tox.getAddressHexSync().toUpperCase().should.equal(
               address.toUpperCase());
         });
       });

    it("should throw if wrong passphrase is given", function() {
      var toxToSave = new Tox(), data = toxToSave.getSavedataSync(),
          edata = toxToSave.crypto().encryptSync(data, "myPassphrase");
      // Should throw TOX_ERR_DECRYPTION_FAILED
      (function() { new Tox({data : edata, pass : "notThePass"}); }
           .should.throw());
    });
  ...
  describe("version functions", function() {
    it("should return numbers", function() {
      var version = [
        tox.versionMajorSync(), tox.versionMinorSync(), tox.versionPatchSync()
      ];
    ...
    it("should return numbers (async)", function(done) {
      async.parallel(
          [
            tox.versionMajor.bind(tox), tox.versionMinor.bind(tox),
  ...
  describe("nospam", function() {
    it("should be a number",
       function() { tox.getNospamSync().should.be.a.Number; });
  ...
  describe("ToxOptions", function() {
    ...
    it("should allocate and free (async)", function(done) {
      tox.newOptions(function(err, opts) {
        if (!err) {
          tox.freeOptions(opts, function(err) { done(err); });
        } else
          done(err);
      });
    });
  ...
  describe("file control name to value", function() {
    it("should convert from control name to control value correctly",
       function() {
  ...
  describe("event emitter", function() {
    it("should be gettable from a Tox instance",
       function() { should.exist(tox.getEmitter()); });
  });
});
